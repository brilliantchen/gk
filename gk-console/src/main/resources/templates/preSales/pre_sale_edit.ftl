          ...
          <div class="box">
            ...
            <div class="box-body">
              <form id="addForm">
                ...
                <#list vo.skus as e>
                  <input type="hidden"  name="skuIds[${e_index}]" value="${e.id}">
                </#list>
                <div class="form-horizontal">
                  <div class="form-group">
                    <label for="" class="col-sm-3 control-label">预售项目名称：</label>
                    <div class="col-sm-5">
                      <input type="text" class="form-control" id="pre_projectName" name="name" value="${vo.name}"
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-5">
                      <input type="text" class="form-control" id="pre_startTime"  value="${vo.startTime?datetime}"
                             placeholder="预售开始时间">
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="col-sm-5">
                      <input type="text" class="form-control" id="pre_productName" name="title"  value="${vo.title}"
                  ...
                  <div class="form-group">
                    ...
                    <#if vo.labels??>
                    <#list vo.labels as e>
                        <label>${e}&nbsp;<a id="#" class="pre_tag">X</a> </label>&nbsp;&nbsp;&nbsp;
                    </#list>
                    </#if>
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="col-sm-3">
                      <input type="text" class="form-control" id="pre_sellPrice" name="sellPrice"  value="${vo.sellPrice}"
                             placeholder="请输入现价（元）">
                    </div>
                    <label for="" class="col-sm-2 control-label">原价：</label>
                    <div class="col-sm-3">
                      <input type="text" class="form-control" id="pre_originalPrice" name="originalPrice"  value="${vo.originalPrice}"
                             placeholder="请输入原价（元）">
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="col-sm-5">
                                            <textarea name="desc" id="pre_desc" cols="" rows="3"
                                                      style="width:100%"> ${vo.desc}</textarea>
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="" class="col-sm-3 control-label">预售产品总量：</label>
                    <div class="col-sm-3">
                      <input type="text" class="form-control" id="pre_stock" name="totalNum"  value="${vo.totalNum?c}"
                             placeholder="请输入预售产品总量">
                    </div>
                    <label for="" class="col-sm-2 control-label">预售警告总量：</label>
                    <div class="col-sm-3">
                      <input type="text" class="form-control" id="pre_alertStock" name="alertStock"  value="${vo.alertStock?c}"
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-5">
                      <input type="text" class="form-control" id="pre_shareUrl" name="shareUrl" value="${vo.shareUrl}"
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-8">
                      <table class="table with-border" id="pre_skus">
                        ...
                        <#list vo.skus as e>
                          <tr><td><img style="width: 180px;" src="${e.img}"></td><td>${e.name}</td><td>${e.price?c}</td><td>${e.stock?c}</td><td>${e.desc}</td><td style="display: none">${e.img}</td><td style="display: none">${e.id}</td><td><button class="btn btn-default btn-xs del_sku"  type="button">删除</button>&nbsp;<button class="btn btn-default btn-xs" type="button" onclick="editsku(this)" >编辑</button></td></tr>
                        </#list>
                      </table>
                    </div>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-5">
                      ...
                      <#list vo.promotions as e>
                        <tr><td>${e.title}</td><td>${e.desc}</td><td><a class="btn btn-default btn-xs del_prom">删除</a></td></tr>
                      </#list>
                      </#if>
                      </table>
                    </div>
                  ...
                  <div class="form-group">
                    ...
                    <input type="hidden" name="coverImg" id="pre_coverImg" value="${vo.coverImg}">
                  </div>
...
<script>
  ...
  $(function () {
    ...
    $(document).on('click','#add_label',function () {
      if($("#label").val() == '') {
        alertify.error("请输入标签!");
        return;
      }
      if($('.pre_tag').length>=3){
        alertify.error("标签数达到上限!");
        return;
      }
    ...
    $(document).on('click','#add_sku',function () {
      ...
      if(skuEleTr == null){
        $("#pre_skus").append('<tr><td><img style="width: 180px;" src='+$("#sku_coverImg").val()+'></td><td>'+$("#sku_name").val()+'</td><td>'+$("#sku_amount").val()+'</td><td>'+$("#sku_num").val()+'</td><td>'+$("#sku_desc").val()+'</td><td style="display: none">'+$("#sku_coverImg").val()+'</td><td style="display: none"></td><td><button class="btn btn-default btn-xs del_sku"  type="button">删除</button>&nbsp;<button class="btn btn-default btn-xs" type="button" onclick="editsku(this)" >编辑</button></td></tr>');
      }else{
        skuEleTr.children()[0].innerHTML = "<img style=\"width: 180px;\" src=\""+$("#sku_coverImg").val()+"\">";
        skuEleTr.children()[1].innerHTML=$("#sku_name").val();
        skuEleTr.children()[2].innerHTML=$("#sku_amount").val();
        skuEleTr.children()[3].innerHTML=$("#sku_num").val();
        skuEleTr.children()[4].innerHTML=$("#sku_desc").val();
        skuEleTr.children()[5].innerHTML=$("#sku_coverImg").val();
      }
      $("#sku_name").val("");
      $("#sku_amount").val("");
      $("#sku_num").val("");
  ...
  var initFileUpload = function () {
    $.ajax("/get_signature", {
      ...
      inp2.fileinput({
        ...
        allowedFileTypes: ['jpg', 'png', 'JPG', 'PNG'],
        initialPreview: [
          "<img style=\"width: 150px\" src='${vo.coverImg}' class='file-preview-image' >",
        ],
        uploadUrl: signature.host,
      }).on('change', function () {
        //inp.fileinput('upload');
  ...
  var initBannerFileUpload = function () {
    ...
    <#if vo.bannerImgs??>
      initialPreview: [
        <#list vo.bannerImgs as e>
          "<img style=\"width: 150px\" src='${e}' class='file-preview-image' >",
        </#list>
      ],
    </#if>
      maxFileSize:2000,
  ...
  var savePreSaleProject = function () {
    ...
    $("#pre_skus > tbody > tr").each(function () {
      ...
      var record = arr[1].innerText+'|'+arr[2].innerText+'|'+arr[3].innerText+'|'+arr[4].innerText+'|'+arr[5].innerText+'|'+arr[6].innerText+'|'+arr[7].innerText;
...
//            } else
  ...
  var editsku = function (t) {
    skuEleTr = $(t).parent().parent();
    var arr = $(t).parent().parent().find("td");
    $("#sku_name").val(arr[1].innerText);
    $("#sku_amount").val(arr[2].innerText);
    $("#sku_num").val(arr[3].innerText);
    $("#sku_desc").val(arr[4].innerText);
    $("#sku_coverImg").val(arr[5].innerText);
    $("#file1").fileinput("refresh",{
   	  initialPreview: [
        "<img style=\"width: 150px\" src='"+arr[5].innerText+"' class='file-preview-image' >",
      ],
    })
  }
